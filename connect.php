...
class PDO_ extends PDO
{
    ...
    {
        $query = $this->prepare("SELECT request.id,request.name,request.description,category.category_name,request.status,request.data,request.id_user FROM request INNER JOIN category  INNER JOIN user WHERE request.id_user=:id_user and category.id=request.id_category and user.id=request.id_user");
